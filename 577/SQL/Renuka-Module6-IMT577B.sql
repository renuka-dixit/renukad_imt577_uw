...
--Inserting data from stage table into dimension table
INSERT INTO Dim_Location (
        PostalCode,
        Address,
        City,
...
SELECT IFNULL(PostalCode, -1),
    IFNULL(Address, 'Unknown'),
    IFNULL(City, 'Unknown'),
    IFNULL(StateProvince, 'Unknown'),
...
CREATE OR REPLACE TABLE Dim_Customer (
        DimCustomerID INT IDENTITY (1, 1) CONSTRAINT PK_CustomerID PRIMARY KEY NOT NULL, --Surrogate Key
...
SELECT IFNULL(DimLocationID, -1),
    IFNULL(CustomerID, 'Unknown'),
    IFNULL(CONCAT(FirstName, ' ', LastName), 'Unknown') as FullName,
    IFNULL(FirstName, 'Unknown'),
    IFNULL(LastName, 'Unknown'),
    IFNULL(Gender, 'Unknown'),
    IFNULL(EmailAddress, 'Unknown'),
    IFNULL(PhoneNumber, 'Unknown')
...
-------------DIM_STORE
--==================================================
--Creating the Dim_Store table
CREATE OR REPLACE TABLE Dim_Store (
        DimStoreID INT IDENTITY (1, 1) CONSTRAINT PK_DimStoreID PRIMARY KEY NOT NULL, --Surrogate Key
        DimLocationID INTEGER CONSTRAINT FK_DimLocationIDStore FOREIGN KEY REFERENCES Dim_Location (DimLocationID) NOT NULL, --Foreign Key
...
VALUES (-1, -1, -1, -1, 'Unknown', 'Unknown');
...
SELECT IFNULL(DimLocationID, -1),
    IFNULL(StoreID, -1),
    IFNULL(StoreNumber, -1),
    IFNULL(StoreManager, 'Unknown'),
    IFNULL(PhoneNumber, 'Unknown')
FROM STAGE_STORE s
    JOIN DIM_LOCATION l
WHERE s.PostalCode = l.PostalCode
    AND s.Address = l.Address;

--===================================================
-------------DIM_RESELLER
--==================================================
--Creating the Dim_Reseller table
CREATE OR REPLACE TABLE Dim_Reseller(
        DimResellerID INT IDENTITY(1, 1) CONSTRAINT PK_DimResellerlID PRIMARY KEY NOT NULL, --Surrogate Key
        DimLocationID INTEGER CONSTRAINT FK_DimLocationIDReseller FOREIGN KEY REFERENCES Dim_Location (DimLocationID) NOT NULL, --Foreign Key
...
SELECT IFNULL(DimLocationID, -1),
    IFNULL(ResellerID, 'Unknown'),
    IFNULL(ResellerName, 'Unknown'),
    IFNULL(Contact, 'Unknown') AS ContactName,
    IFNULL(PhoneNumber, 'Unknown'),
    IFNULL(EmailAddress, 'Unknown')
FROM STAGE_RESELLER r
    JOIN DIM_LOCATION l
WHERE r.PostalCode = l.PostalCode
    AND r.Address = l.Address;
...
SELECT IFNULL(ChannelID, -1),
    IFNULL(c.ChannelCategoryID, -1),
    IFNULL(Channel, 'Unknown') AS ChannelName,
    IFNULL(ChannelCategory, 'Unknown')
...
-------------DIM_PRODUCT
--==================================================
--Creating the Dim_Product table
...
SELECT IFNULL(ProductID, -1),
    IFNULL(p.ProductTypeID, -1),
    IFNULL(pt.ProductCategoryID, -1),
    IFNULL(Product, 'Unknown'),
    IFNULL(ProductType, 'Unknown'),
    IFNULL(ProductCategory, 'Unknown'),
    IFNULL(Price, -1.0),
    IFNULL(WholesalePrice, -1.0),
    IFNULL(Cost, -1.0),
    IFNULL((Price - Cost), -1.0) AS ProductRetailProfit,
    IFNULL((WholesalePrice - Cost), -1.0) AS ProductWholesaleUnitProfit,
    IFNULL((((Price - Cost) / Price)), -1.0) AS ProductProfitMarginUnitPercent
FROM STAGE_PRODUCT p
    JOIN STAGE_PRODUCTTYPE pt
    JOIN STAGE_PRODUCTCATEGORY pc ON p.ProductTypeId = pt.ProductTypeID
